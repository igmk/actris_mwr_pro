...
offset_tb,$                     ;possible tb offset correction (ig desired) 
offset_tb_type,$                ;CHARACTER specifying source for offset correction
...
; $Id: $
; Abstract:
; * write HATPRO level1b TB files to netcdf format
; Authors:
; U. Loehnert
; Date:
; 2011-09-03
...
tbocrid = NCDF_VARDEF(outfid, 'offset_tb',[fredid,timdid], /float)
...
tbbid = NCDF_VARDEF(outfid, 'tb_bias',[fredid], /float)
NCDF_ATTPUT, outfid, tbbid, 'units', 'K'
NCDF_ATTPUT, outfid, tbbid, 'long_name', 'systematic calibration uncertainty of brightness temperature, one standard deviation'
...
NCDF_VARPUT, outfid, tbnid, TRANSPOSE(time_bnds)
...
NCDF_VARPUT, outfid, tbbid, tb_bias
NCDF_VARPUT, outfid, tbcid, tb_cov
IF n_wavel_irt GT 0 THEN BEGIN
 NCDF_VARPUT, outfid, wirid, wavel_irt
 NCDF_VARPUT, outfid, tbiid, tb_irt+273.15d
 NCDF_VARPUT, outfid, eliid, el_irt
ENDIF
