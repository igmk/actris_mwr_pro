...
def apply_met_qc(data: dict, 
    ...
    var_name = ['air_temperature', 'relative_humidity', 'air_pressure', 'rain_rate', 'wind_direction', 'wind_speed']
    
    for bit, name in enumerate(var_name):
        if name in data:
            ind = np.where((data[name][:] < params['met_thresholds'][bit][0]) | (data[name][:] > params['met_thresholds'][bit][1]))
