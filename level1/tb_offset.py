...
def correct_tb_offset(site: str, 
                      data: dict, 
                      params: dict,
...
def cal_nex(params: dict,
            time1: int,
            rpg_cal: dict) -> int:
    """Load and add information from ABSCAL.HIS file"""
    
    cal_t = time1
    rec_cal = np.where((rpg_cal.data['cal1_t'] == 1) & (rpg_cal.data['cal2_t'] == 1) & (epoch2unix(rpg_cal.data['t1'], rpg_cal.header['_time_ref']) > time1))[0]
    if len(rec_cal) > 0:
