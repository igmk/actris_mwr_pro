...
date = datetime.date(int(dvec[0:4]), int(dvec[4:6]), int(dvec[6:8]))
...
for prod, var in product:
    if prod == '1C01':
        ...
        lev1_to_nc(site, prod, params['data_in']+date.strftime('%Y/%m/%d/'), params['data_in']+prv.strftime('%Y/%m/%d/'), params['data_in']+nex.strftime('%Y/%m/%d/'), lev1_data)
        if do_plot:
            generate_figure(lev1_data, ['tb'], ele_range = [89., 91.], save_path=data_out_l1, image_name='tb')
            generate_figure(lev1_data, ['ele', 'azi'], save_path=data_out_l1, image_name='sen')
            generate_figure(lev1_data, ['quality_flag'], save_path=data_out_l1, image_name='quality_flag')
            generate_figure(lev1_data, ['air_temperature', 'relative_humidity', 'air_pressure', 'rain_rate', 'wind_direction', 'wind_speed'], save_path=data_out_l1, image_name='met') 
            if params['ir_beamwidth'] != -999.:
                generate_figure(lev1_data, ['irt'], ele_range = [89., 91.], save_path=data_out_l1, image_name='irt')
    else:
        if not os.path.isdir(data_out_l2):
            os.makedirs(data_out_l2)
        lev2_to_nc(site, prod, data_out_l1+'MWR_1C01_'+ID+'_'+date.strftime('%Y%m%d')+'.nc', data_out_l2+'MWR_'+prod+'_'+ID+'_'+date.strftime('%Y%m%d')+'.nc')
        if do_plot:
            generate_figure(data_out_l2+'MWR_'+prod+'_'+ID+'_'+date.strftime('%Y%m%d')+'.nc', [var], save_path=data_out_l2, image_name=var)
