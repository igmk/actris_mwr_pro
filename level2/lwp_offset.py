...
def correct_lwp_offset(lev1: dict,
    ...
    if ind.size > 1:
        off = off.append(pd.DataFrame({'date': time[ind[0]], 'offset': lwp_offset['Lwp'][ind[0]]}, index = {0}), ignore_index = True)
        off = off.append(pd.DataFrame({'date': time[ind[-1]], 'offset': lwp_offset['Lwp'][ind[-1]]}, index = {0}), ignore_index = True)
    elif ind.size == 1:
        off = off.append(pd.DataFrame({'date': time[ind], 'offset': lwp_offset['Lwp'][int(ind)]}, index = {0}), ignore_index = True)
