...
def get_mvr_coeff(site: str, prefix: str, freq: np.ndarray):
    ...
    if (str(c_list[0][-3:]).lower() == "ret") & (len(c_list) == 1):
        ...
        for i, file in enumerate(c_list):
            with open(file, "r") as f:
                ...
                for il, line in enumerate(lines):
                    ...
                    if ls[0] == "AL":
                        cf["height_grid"] = np.array([float(x) for x in ls[1].split()], np.float32)
                        cf["n_height_grid"] = len(cf["height_grid"])
                    ...
                    if cf["rt"] == 2:
                        ...
                        if ls[0] == "NP":
                            if "np" not in cf:
                                cf["np"] = np.ones(len(cf["freq"]), np.float32) * Fill_Value_Float
                                cf["np"][0] = float(ls[1][:].split()[0])
                            else:
                        ...
                        if ls[0] == "NS":
                            ...
                            if "op_os" not in cf:
                                if prefix == 'tel':
                                    nn = len(freq_cf) * len(cf["ele"]) + np.sum(cf["aux_flg"]) + 1
                                    cf["freq_bl"] = cf["freq"]
                                else:
                                    nn = len(freq_cf) + np.sum(cf["aux_flg"]) + 1                                
                                cf["in_os"] = np.ones((1, nn), np.float32) * Fill_Value_Float
                                cf["in_sc"] = np.ones((1, nn), np.float32) * Fill_Value_Float
                                cf["op_os"] = np.ones((1, len(cf["height_grid"])), np.float32) * Fill_Value_Float
                                cf["op_sc"] = np.ones((1, len(cf["height_grid"])), np.float32) * Fill_Value_Float
                                cf["in_os"][0, :] = np.array([float(idx) for idx in cll], np.float32)
                                cll = lines[il + 1].split(":")[1].split()
                                cf["in_sc"][0, :] = np.array([float(idx) for idx in cll], np.float32)
                                cll = lines[il + 2].split(":")[1].split()
                                cf["op_os"][0, :] = np.array([float(idx) for idx in cll], np.float32)
                                cll = lines[il + 3].split(":")[1].split()
                                cf["op_sc"][0, :] = np.array([float(idx) for idx in cll], np.float32)
                            else:
                                cf["in_os"] = np.vstack(
                                    (
                                        cf["in_os"],
                        ...
                        if ls[0] == "W2":
                            cll = ls[1].split()
                            if "w2" not in cf:
                                cf["w2"] = np.ones((1, cf["nd"][0] + 1, len(cf["height_grid"])), np.float32) * Fill_Value_Float
                                for ialt, _ in enumerate(cf["height_grid"]):
                                    cll = lines[il + ialt].split()[1:cf["nd"][0]+2]
                                    cf["w2"][0, :, ialt] = np.array(
                                        [float(idx) for idx in cll[0 : cf["nd"][0] + 1]],
                                        np.float32,
                                    )
                            else:
                                cf["w2"] = np.vstack(
                                    (
                                        ...
                                        np.array(
                                            [float(idx) for idx in cll[0 : cf["nd"][0] + 1]],
                                            np.float32,
                                        ), (1, cf["nd"][0] + 1, len(cf["height_grid"]))),
                                    )
                                )
        ...
        def in_sc(x):
            return np.array([cf["in_sc"][(np.abs(cf["ele"] - v)).argmin(), :] for v in x])

        def in_os(x):
            return np.array([cf["in_os"][(np.abs(cf["ele"] - v)).argmin(), :] for v in x])

        def op_sc(x):
            return np.array([cf["op_sc"][(np.abs(cf["ele"] - v)).argmin(), :] for v in x])

        def op_os(x):
            return np.array([cf["op_os"][(np.abs(cf["ele"] - v)).argmin(), :] for v in x])            
    ...
    return (
        ...
        if (cf["rt"] < 2)
        else (cf, in_sc, in_os, op_sc, op_os, W1, W2, pn)
    )
